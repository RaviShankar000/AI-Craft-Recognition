...
class ChatbotService {
  ...
  /**
   * Use LLM to answer cultural questions about crafts with streaming support
   ...
   */
  async answerCulturalQuestion(question, onToken = null) {
    try {
      ...
      if (
        ...
        const errorResponse = this.createFormattedResponse(
        ...
        // Stream error message if callback provided
        if (onToken && errorResponse.text) {
          await this.streamText(errorResponse.text, onToken);
        }
...
Use engaging language that celebrates the artistry and cultural heritage.`;
      ...
      if (onToken) {
        ...
        // Stream prefix
        const prefix = '🤖 **AI Cultural Expert**\n\n';
        ...
        const formattedAnswer = `${prefix}${fullAnswer}${suffix}`;
  ...
  /**
   * Process user message and generate response with streaming support
   * @param {String} message - User message
   * @param {Object} _context - Conversation context (unused but kept for future enhancements)
   * @param {Object} callbacks - Streaming callbacks (onToken, onComplete, onError)
   * @returns {Object} Response with text and suggestions
   */
  async processMessage(message, _context = {}, callbacks = {}) {
    ...
    try {
      ...
      if (intent !== 'cultural' && intent !== 'indianCraft') {
        ...
        if (cachedResponse) {
          ...
          if (onToken && cachedResponse.text) {
            await this.streamText(cachedResponse.text, onToken);
          }
      ...
      // If cultural or Indian craft question detected, use LLM with streaming
      if (intent === 'cultural' || intent === 'indianCraft') {
        const llmResponse = await this.answerCulturalQuestion(sanitizedMessage, onToken);
      ...
      if (onToken && response.text) {
        await this.streamText(response.text, onToken);
      }
  ...
  /**
   * Stream text word by word with delays for typing effect
